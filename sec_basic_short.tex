...
In this work, we focus only on HL; for analogous results for CH refer to \cite{TechReport}. % Appendix~\ref{sec:ch}.
...
In \cref{ssec:aug} we show that, if the HD is $h$, in polynomial time we can obtain sparsity $h'=\Or(h\Delta\log(h\Delta))$.
