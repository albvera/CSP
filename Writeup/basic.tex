...
The length $\ell(P)$ is the sum of edge lengths in $P$. We define a path with a single node $v$ to have length zero \anote{I think single nodes should not be considered paths, they are only relevant for the stronger version of HD }; note also that any path $P$ with more than one node has length at least $1$ (since $\ell:E\to \N_+$).
