...
\sbedit{Comparing the size of the \emph{optimal} hub labels for SP and CSP is infeasible as even finding the optimal hub labels for SP is NP-hard~\cite{babenko_hl_complexity}. 
...
However, we now show that in general, the CHD, and in fact, the sparsity of any EPHS, can be \emph{arbitrarily worse} than the HD. 
\begin{proposition}
...
We now show that if costs are such that the graph indeed obeys the partial witness property (for long-enough paths) for some $\beta$, then we can relate the CHD and HD in terms of $\beta$ and the doubling dimension $\alpha$. Note that the doubling dimension depends only on $G$ and $\ell$; the partial witness property depends on the interplay between $G$, $c$ and $\ell$.
...
\anote{There is no relation between the quantities $\alpha$ and $\Delta$ in the sense that neither bounds the other.}
