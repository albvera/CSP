...
%For each maximum budget level $B$, the results: i) $B-f$ that computes the entire solution frontier for all budgets $b\leq B$ on a fully pruned graph (as given in Definition~\ref{def:pruned_aug_graph}, and ii) $B-s$ that computes the solution only for the specified budget level on the fully pruned graph with XXX (\todo{Alberto to connect with previous section}).
