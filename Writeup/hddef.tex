...
We define a \emph{path system} $\calQ$ as any \sbedit{downward-closed} collection of paths
\sbedit{(by downward-closed, we mean that if $P\in\calQ$ and $P'\subset P$, then $P'\in\calQ$)}.
...
\sbnote{Need an algorithm + lemma here}

To find hitting sets which are independent of $n$,~Abraham et al.~\cite{highway2013} introduced the notion of the \emph{highway dimension}. We first need two additional definitions:
...
\end{proposition}
This follows directly from Theorem 4.2 in \cite{highway2013}, which showed it for the shortest-path system. Moreover, as we discuss in Section~\ref{sec:preproc}, if $G$ has HD$\leq h$, then this permits the construction of a $(h\log h,r)$-LSHS in polynomial time.
