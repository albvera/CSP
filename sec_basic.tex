...
We denote the shortest $(s,t)$-path (if it exists) \SA{Do any of the results require the graph to be strongly connected?} as $P(s,t)$, and denote the set of all shortest paths in $G$ as $\PS$.
...
\end{definition}
We denote the set of all efficient paths as $\PE$, and define the \emph{Pareto frontier} \SA{of efficient paths?} 	from $s$ to $t$ as $\Pst\cap\PE$.
