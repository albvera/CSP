...
Given a source-terminal pair $s,t$ and a budget $b$, we want to return a path solving
$
\min\crl{\ell(P) : P\in\Pst, c(P) \leq b}.
$
...
To limit these solutions, we require that the path also be \emph{efficient}. 
