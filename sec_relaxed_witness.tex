...
To do this, we first allow for an additional \emph{circumstantial witness set} $D$ such that \emph{every efficient path is either witnessed by a shortest-path or hit by $D$}.
...
Intuitively, the circumstantial witness set $D_r$ takes care of all the corner cases where $\calQ$ and $\calQ'$ differ too much.
...
We will show how to obtain a vertex for the circumstantial witness set in case $Q$ is not witnessed and then we bound the size of the set.
...
For every path of length $2r$, we can thus either exhibit a witness or show that it contains an overpass and add use the tail of the edge as a circumstantial witness.
