...
Formally, we define:% desirable property of an algorithm.
...
% -- edges connecting long shortest paths in a costly zone.


\begin{definition}[Overpass]
%For $r>0$, an 
Edge $e=(u,v)$ is an $r$-overpass if:\\
...
We henceforth refer to the average HD of $\PE$ as average CHD; since the average HD is enough to obtain good preprocessing algorithms (cf. \cref{theo:preproc_avg}), therefore an average CHD should intuitively suffice.
...
To do this, we first allow for an additional \emph{residual witness set} $D$ such that \emph{every efficient path is either witnessed by a shortest-path or hit by $D$}.
...
Intuitively, the residual witness set $D_r$ takes care of all the corner cases where $\calQ$ and $\calQ'$ differ too much.
...
We will show how to obtain a vertex for the residual witness set in case $Q$ is not witnessed and then we bound the size of the set.
...
Condition 1 is clearly satisfied. Condition 2 also holds because either $Q[s,v]$ or $Q[u,t]$ have stretch at most $\frac{2-\St}{\St}$.
...
For every path of length $2r$, we can thus either exhibit a witness or show that it contains an overpass and add use the tail of the edge as a residual witness.
...
The last piece is \cref{prop:avg_chd}, the proof of which follow form similar arguments as those in \cref{theo:HLeff,theo:preproc_avg}.
%is far from trivial, but we already gave the construction with the augmented graph.
