...
Our work contributes to a growing field of algorithms for large-scale problems (non-convex methods, sketching techniques, etc.), with relatively \SA{Added relatively here because poor for SP is quite fast compared to other problems.} poor worst-case performance, but which are provably efficient for practically relevant settings.
...
This can be addressed by computing shortest paths based on \emph{probabilistic} \SA{changed to probabilistic because robust may imply worst case.} travel-time estimates:
...
Computing this exactly for general distributions is expensive due to the need for computing convolutions of distributions \SA{There is also no polynomial time algorithm for doing this even if you had a black box for solving the convolutions.}. 
...
Using this, we can reformulate the robust trip-time estimation problem \SA{This is not a trip-time estimation problem, but a path optimization problem. Also, I would once again not use robust for the reasons mentioned above.} as 
