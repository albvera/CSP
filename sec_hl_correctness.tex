...
We now explain the construction for HL; for the construction and results of CH refer to an appendix in our full-version.
...
Although hub labels always exist (in particular, we can always choose $\Lb(s)=\Lf(s)=V$), finding \emph{optimal} hub-labels in terms of storage/query-time is known to be NP-hard~\cite{babenko_hl_complexity}.
...
Finally, any point in the path belongs to both $\Bf_{2^i}(s)$ and $\Bb_{2^i}(t)$, hence $C_i\cap P(s,t)$ is in both hubs (which is not empty since $C_i$ hits all SP of length $\geq 2^{i-1}$).
\end{proof}

All of our constructions rely on the LSHS structures.
We describe how to obtain these objects in polynomial time in our full version.
Since our main contributions are not on this regard, in what follows we assume the multi-scale LSHS is given.
For space constraints, we have omitted several proofs, these can be found in our full version. 
