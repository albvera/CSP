...
\respedit{Hub Labels must satisfy the \emph{cover property} defined as follows: for any $s\neq t\in V$, $\Lf(s)\cap\Lb(t)$ contains at least one node in $P(s,t)$.
...
On the other hand, when $P(s,t)$ exists, \respedit{ we do a case analysis on $\ell(P)$ to prove the cover property.}
