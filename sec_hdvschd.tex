...
Therefore, the hitting set $H_{s,1}$ must contain at least $n$ elements. Moreover, the same argument shows that the sparsity of the best LSHS for $\PS$ is $1$, whereas the sparsity of \emph{any} EPHS is also lower bounded by $n$. \SA{It seems strange to be comparing the best LSHS to any EPHS.} 
\end{proof}
...
Then, for any $r>0$, given an $(h,r)$-LSHS, we can construct, in polynomial time, an $(\alpha^{\beta} h,r)$-EPHS for $(G,\ell,c)$. \SA{Would be nice to be consistent with $\alpha^{\beta}h$ vs $h\alpha^{\beta}$. The latter is used most of the time. } 
\end{theorem}
