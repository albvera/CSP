...
In particular, we use the \emph{highway dimension}, introduced by \citep{hd_journal,highway2010} to parametrize shortest-path computations in undirected graphs. 
A few of our results are generalizations of those in \citep{hd_journal}; the technical challenges of these extensions may not be clear to a non-expert reader, thus we differ all discussions on the matter to \cref{app:generalhd}. 
Very broadly speaking, \cite{hd_journal} deals only with undirected graphs and shortest paths, whereas our approach covers directed graphs and general sets of paths.
...
We outline this construction for directed graphs, closely following the construction in \cite[Theorem 5.1]{hd_journal} for the undirected case.
\begin{proposition}
...
The original definition of HD was given by \citep{highway2010}. This is not the one we extend, but rather we work from the definition of~\citep{hd_journal}.
...
\begin{theorem}
Assume that $G$ has doubling constant $\alpha$ and $\calQ,\calQ'$ are systems with HD $h,h'$ respectively. 
...
\bibitem[{Abraham et~al.(2013)Abraham, Delling, Fiat, Goldberg,
  \protect\BIBand{} Werneck}]{hd_journal}
